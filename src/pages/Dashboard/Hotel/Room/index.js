...
export default function Room_Options({ name, capacity, id, booking, selectedRoom, setSelectedRoom }) {
  ...
  if( capacity === 1 && bookingByRoomId.length === 0) {
    return(
      <Room_Box_Avalible onClick={() => {setSelectedRoom(id); setIonIconSelected(id); }} selectedRoom={selectedRoom} roomId={id}>
        <div>{name}</div>
        {ionIconSelected !== selectedRoom ? <ion-icon  name="person-outline"></ion-icon> : <ion-icon  style={{ color: '#FF4791' }}  name="person"></ion-icon> }
      </Room_Box_Avalible>
    );
...
margin: 0px 16px 16px 0px;
padding: 14px;
background: ${props => props.selectedRoom === props.roomId ? '#FFEED2' : 'white'};
size: 24px;
